"""Génération de la facture PDF d'achat avec ReportLab."""
import io
...
def generer_facture_pdf(achat):
    ...
    header_data = [[
        ...
        Paragraph("FACTURE D'ACHAT", styles["FactureTitle"]),
    ]]
    ...
    meta_data = [[
        Paragraph(f"<b>N° Facture :</b> {achat.numero_facture}", styles["Small"]),
        Paragraph(f"<b>Date :</b> {achat.date_achat.strftime('%d/%m/%Y')}", styles["SmallRight"]),
    ]]
    elements.append(Table(meta_data, colWidths=[100 * mm, 72 * mm]))
    elements.append(Spacer(1, 6 * mm))

    # --- Bloc fournisseur ---
    fournisseur = achat.fournisseur
    lignes = ["<b>Fournisseur :</b>", fournisseur.nom]
    if fournisseur.telephone:
        lignes.append(fournisseur.telephone)
    if fournisseur.ville or fournisseur.adresse:
        lignes.append(f"{fournisseur.adresse} {fournisseur.ville}".strip())
    elements.append(Paragraph("<br/>".join(lignes), styles["Small"]))
    ...
    table_data.append([
        achat.materiau.nom,
        f"{achat.quantite:.2f}",
        f"{achat.prix_unitaire:.2f}",
        f"{achat.montant_total:.2f}",
    ])
    ...
    # --- Total ---
    total_data = [["", "Montant total", f"{achat.montant_total:.2f} DH"]]
    ...
    elements.append(Paragraph(f"N° bon de pesée : {achat.numero_bon}", styles["Small"]))
    doc.build(elements)
    buffer.seek(0)
    return buffer


def generer_facture_groupee_pdf(achats):
    """Génère un relevé PDF regroupant plusieurs achats avec un total général."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer, pagesize=A4,
        topMargin=20 * mm, bottomMargin=20 * mm, leftMargin=18 * mm, rightMargin=18 * mm,
    ...
    header_data = [[
        ...
        Paragraph("RELEVÉ D'ACHATS", styles["FactureTitleG"]),
    ]]
    header_table = Table(header_data, colWidths=[100 * mm, 72 * mm])
    header_table.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP")]))
    elements.append(header_table)
    ...
    elements.append(Paragraph(f"<b>Nombre de factures :</b> {len(achats)}", styles["SmallG"]))
    elements.append(Spacer(1, 8 * mm))

    table_data = [["Facture", "Fournisseur", "Matière", "Date", "Quantité", "Prix unit. (DH)", "Montant (DH)"]]
    total_general = 0
    for a in achats:
        table_data.append([
            a.numero_facture,
            ...
            f"{a.quantite:.2f} Kg",
            f"{a.prix_unitaire:.2f}",
            f"{a.montant_total:.2f}",
        ])
        total_general += a.montant_total
